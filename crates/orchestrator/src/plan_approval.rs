...
mod tests {
    ...
    fn routing_execute() -> RouterOutput {
        // `RouterOutput` is `#[non_exhaustive]`; build the fixture through its
        // serde representation now that the deterministic `route()` constructor
        // is gone.
        serde_json::from_value(serde_json::json!({
            "outcome": "execute",
            "scope": { "project_dir": "/tmp", "hinted_paths": [] },
            "confidence": 0.8,
            "route": { "RuleHit": { "rule": "execute_keyword" } },
        }))
        .expect("valid RouterOutput fixture")
    }
